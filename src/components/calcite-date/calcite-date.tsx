...
export class CalciteDate {
  ...
  private renderMonths(month, year) {
  ...
  private renderDay(day, cname, isSelected = false) {
    return (
      <span
  ...
  private renderCalendar(month, year, selectedDate) {
    let weekDays = this.getLocalizedWeekday(),
      ...
      splitDays = [],
      days = [
        ...prevMonDays.map(prev => this.renderDay(prev, "previous-month-day")),
        ...curMonDays.map(cur =>
          this.renderDay(cur + 1, "current-month-day", cur + 1 === selectedDate)
        ),
        ...nextMonDays.map(next => this.renderDay(next + 1, "next-month-day"))
      ];
